...
multiprocessing モジュールは複数プロセスで分散処理を扱う比較的シンプルな API を提供します。multiprocessing モジュールは :mod:`threading` の API をベースにしていて場合によっては暫定的に置き換えられます。2つのモジュールがよく似ているので、最初の数個のサンプルは threading のサンプルから修正しています。multiprocessing で提供される機能は threading では利用できないことを後で説明します。
